...
ipcMain.handle('orchestra:clearLog', (_e, dir) => {
  ...
  _metricsCache.delete('claude-usage:' + dir)
  _metricsCache.delete('compliance:' + dir)
  _metricsCache.delete('freshness:' + dir)
  _metricsCache.delete('session-summary')
  usageTracker.delete(dir)
  ...
  return new Promise(resolve => {
    ...
    execFile('git', ['-C', dir, 'log', '--oneline', '--since', started || '30 days ago'], { timeout: 8000, maxBuffer: 262_144 }, (gitErr, gitOut) => {
      ...
      const report = [
        ...
        (() => { const _orParts = []; for (const l of _orchestraReport.split('\n').slice(-150)) _orParts.push(l.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '')); return _orParts.join('\n') })(),
        ``,
        `--- ROADMAP.md ---`, read('ROADMAP.md').replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '') || '(missing)',
        ``,
        `--- PENDING.md ---`, read('PENDING.md').replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '') || '(missing)',
        ``,
        `--- PLAN.md (last 80 lines) ---`,
        (() => { const _plParts = []; for (const l of read('PLAN.md').split('\n').slice(-80)) _plParts.push(l.replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, '')); return _plParts.join('\n') })(),
        ``,

...
- [ ] **P-F09** Notion integration: connect a Notion workspace to a project so the orchestrator can pull pages/databases (feature specs, bug lists, user stories) into ROADMAP.md, BLUEPRINT.md, and PLAN.md generation. Settings panel for Notion API key + database selector. IPC: `integration:notion:connect`, `integration:notion:sync`. Writes discovered items to `.claude/external-backlog.json`.
- [ ] **P-F10** Obsidian vault integration: link a local Obsidian vault folder to a project. The orchestrator reads markdown notes (tagged with `#feature`, `#bug`, `#spec`) and feeds them into roadmap and blueprint generation. Settings panel for vault path + tag filters. IPC: `integration:obsidian:connect`, `integration:obsidian:sync`. Parses frontmatter + wikilinks to discover feature dependencies.
